...
namespace YouthSpice.CookingScene.SelectionStage.UI
{
	...
	public class ElementsManager : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			currentParent = Instantiate(horizontalElementGroupPrefab, Vector3.zero, Quaternion.identity, rootParent).transform;
			Element child = Instantiate(selectNoneElementPrefab, Vector3.zero, Quaternion.identity, currentParent).GetComponent<Element>();
		...
		private void Start()
		{
			...
			foreach (int item in GameInfo.Instance.inventory)
			{
				...
				Element child = Instantiate(elementPrefab, Vector3.zero, Quaternion.identity, currentParent).GetComponent<Element>();
		...
		private void Select(int majorIndex, int minorIndex)
		{
			...
			selectionManager.AddItem(target.ItemNumber, () =>
			{
				ReturnItem(majorIndex, minorIndex);
			});
			// -1은 선택 포기
